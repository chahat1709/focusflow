    ...
    try {
        ...
        if ($fv) {
            if (d.headband_on === false && state.connected) {
                ...
                if ($fl) $fl.textContent = 'Calibrating baseline (60s)';
            } else {
...
function updateLiveCharts() {
    if (focusChart) {
        // Fix #11: Decimate 1500 → ~300 points for Chart.js rendering
        // Keeps full session in memory but only renders every Nth point
        const len = state.timestamps.length;
        const step = Math.max(1, Math.floor(len / 300));
        const labels = [], aD = [], bD = [], tD = [], dD = [], gD = [];
        for (let i = 0; i < len; i += step) {
            labels.push(i);
            aD.push(state.alpha[i]);
            bD.push(state.beta[i]);
            tD.push(state.theta[i]);
            dD.push(state.delta[i]);
            gD.push(state.gamma[i]);
        }
        focusChart.data.labels = labels;
        focusChart.data.datasets[0].data = aD;
        focusChart.data.datasets[1].data = bD;
        focusChart.data.datasets[2].data = tD;
        focusChart.data.datasets[3].data = dD;
        focusChart.data.datasets[4].data = gD;
...
function endSession() {
    ...
    // Stress Fix S5: Cap localStorage to 50 sessions to prevent QuotaExceededError
    ...
    try {
        ...
        // QuotaExceededError protection — clear old data rather than crashing
        console.warn('localStorage quota exceeded, trimming sessions:', e);
